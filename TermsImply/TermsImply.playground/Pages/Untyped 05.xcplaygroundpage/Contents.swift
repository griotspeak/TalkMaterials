...
enum Result<T : CustomStringConvertible> : CustomStringConvertible {
    case success(T)
    case failure(Swift.Error)
    init( _ closure : @autoclosure () throws -> T ) {
        do {
...
struct Environment {
    ...
    func evaluateR(_ term: Term) -> Result<Value> {
        return Result(try evaluate(term))
    }
...
environment.evaluateR(term1)
environment.evaluateR(.application(term1, argument: 2))
environment.evaluateR(.application(term1, argument: 0))
environment.evaluateR(.application(term1, argument: vx))
